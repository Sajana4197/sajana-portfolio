...
export default function Taskbar() {
  ...
  const {
    ...
    openWindow, focusWindow, minimizeWindow,
  ...
  const extraWindows   = windows.filter(w => !PINNED.find(p => p.id === w.id));
  ...
  function handlePinnedClick(item) {
    if (item.id === "start")  { toggleStartMenu(); return; }
    if (item.id === "search") { setSearchOpen(s => !s); return; }
    if (item.id === "github") { window.open("https://github.com/Sajana4197", "_blank"); return; }
    if (["vscode","figma","explorer"].includes(item.id)) return;
  ...
  const searchResults = ICON_DEFS.filter(d =>
    d.label.toLowerCase().includes(searchVal.toLowerCase()) && searchVal.length > 0
  );
        ...
        {searchOpen && (
          ...
          >
            ...
            <div className="flex items-center gap-3 px-4 py-3"
              ...
              <input
                ...
                style={{
                  ...
                  color: "#e8e8f0", fontSize: 16, fontFamily: "'Segoe UI', sans-serif",
                }}
              />
              {searchVal && (
            ...
            <div className="p-3">
              {searchVal.length === 0 ? (
                <div>
                  <p style={{ fontSize: 12, color: "#8888aa", padding: "4px 8px 8px", textTransform: "uppercase", letterSpacing: "0.08em" }}>
                    Quick launch
                  </p>
                  <div className="grid grid-cols-4 gap-2">
                    {ICON_DEFS.slice(0, 8).map(def => (
                      <QuickItem key={def.id} def={def}
                        onClick={() => {
                          if (def.component) openWindow(def.id, def.label, def.component, def.icon);
                ...
                <div>
                  ...
                  {searchResults.map(def => (
                    ...
                    >
                      ...
                      <span style={{ fontSize: 14, color: "#e8e8f0" }}>{def.label}</span>
                    </div>
                  ))}
                </div>
              ) : (
                <p style={{ fontSize: 14, color: "#8888aa", textAlign: "center", padding: "20px 0" }}>
      ...
      >
        ...
        >
            ...
            return (
              <div key={item.id} style={{ position: "relative", display: "flex", flexDirection: "column", alignItems: "center" }}>
                ...
                <motion.button
                  ...
                  style={{
                    width: ICON_BTN_W, height: ICON_BTN_W,
                    display: "flex", alignItems: "center", justifyContent: "center",
                    background: isActive
                      ? "rgba(0,120,212,0.25)"
                      : isHov ? "rgba(255,255,255,0.10)" : "transparent",
          ...
          {extraWindows.map(win => {
            ...
            return (
              <div key={win.id} style={{ position: "relative", display: "flex", flexDirection: "column", alignItems: "center" }}>
                <AnimatePresence>
                  {isHov && (
                    <motion.div
                      style={{
                        ...
                        fontSize: 12, color: "#e8e8f0", whiteSpace: "nowrap",
                        backdropFilter: "blur(12px)", pointerEvents: "none", zIndex: 9999,
                      }}
                      initial={{ opacity: 0, y: 4 }}
                      animate={{ opacity: 1, y: 0 }}
                      exit={{    opacity: 0, y: 4 }}
                ...
                <motion.button
                  ...
                  onMouseLeave={() => setHoverId(null)}
                  onClick={() => {
                    if (win.minimized || activeWindowId !== win.id) focusWindow(win.id);
                    else minimizeWindow(win.id);
                  }}
                  style={{
                    width: ICON_BTN_W, height: ICON_BTN_W,
                    display: "flex", alignItems: "center", justifyContent: "center",
                    background: isActive ? "rgba(0,120,212,0.25)" : isHov ? "rgba(255,255,255,0.10)" : "transparent",
                    border: "none", borderRadius: 8, cursor: "pointer",
                    fontSize: 20, position: "relative",
                  }}
                  whileHover={{ scale: 1.12 }}
    ...
    <div className="flex items-center gap-1 px-2 py-1 rounded-lg"
      ...
      <TrayIcon title="Network">
        <svg width="14" height="14" viewBox="0 0 18 18" fill="none">
          <path d="M1 13.5C3.5 11 6.1 9.8 9 9.8s5.5 1.2 8 3.7" stroke="white" strokeOpacity="0.7" strokeWidth="1.4" strokeLinecap="round"/>
          <path d="M3.5 11C5.3 9.4 7 8.6 9 8.6s3.7.8 5.5 2.4" stroke="white" strokeOpacity="0.7" strokeWidth="1.4" strokeLinecap="round"/>
          <circle cx="9" cy="15" r="1.2" fill="white" fillOpacity="0.7"/>
        </svg>
      </TrayIcon>
      <TrayIcon title="Volume">
        <svg width="14" height="14" viewBox="0 0 18 18" fill="none">
          <path d="M3 6.5h2.5L9 3.5v11L5.5 11.5H3z" stroke="white" strokeOpacity="0.7" strokeWidth="1.2" strokeLinejoin="round"/>
          <path d="M11.5 6a3.5 3.5 0 010 6" stroke="white" strokeOpacity="0.7" strokeWidth="1.2" strokeLinecap="round"/>
        </svg>
      </TrayIcon>
      <TrayIcon title="Battery — 100%">
        <svg width="14" height="14" viewBox="0 0 18 18" fill="none">
          <rect x="1.5" y="5.5" width="13" height="7" rx="1.5" stroke="white" strokeOpacity="0.7" strokeWidth="1.2"/>
          <rect x="3" y="7" width="8" height="4" rx="0.5" fill="white" fillOpacity="0.7"/>
          <path d="M15.5 7.5v3" stroke="white" strokeOpacity="0.7" strokeWidth="1.4" strokeLinecap="round"/>
        </svg>
      </TrayIcon>
    </div>
  );
  ...
  return (
    <button title={title}
      ...
      style={{
        ...
        border: "none", borderRadius: 6, cursor: "pointer", transition: "background 0.12s",
      }}>
      {children}
    </button>
  );
...
function ShowDesktopBtn() {
  ...
  return (
    <button title="Show desktop"
      ...
      onMouseLeave={() => setHov(false)}
      style={{
        width: 6, height: 32, marginLeft: 4,
        background: hov ? "rgba(255,255,255,0.25)" : "rgba(255,255,255,0.10)",
        border: "none", borderRadius: 2, cursor: "pointer", transition: "background 0.12s",
      }}
    />
  );
}
...
function QuickItem({ def, onClick }) {
  ...
  return (
    <button onClick={onClick}
      ...
      <span style={{ fontSize: 11, color: "#e8e8f0", textAlign: "center" }}>{def.label}</span>
    </button>
  );
}
...
function WindowsLogoSmall() {
  return (
    <svg width="18" height="18" viewBox="0 0 88 88" fill="none">
      <rect x="4"  y="4"  width="37" height="37" rx="3" fill="white" fillOpacity="0.9"/>
      <rect x="47" y="4"  width="37" height="37" rx="3" fill="white" fillOpacity="0.9"/>
      <rect x="4"  y="47" width="37" height="37" rx="3" fill="white" fillOpacity="0.9"/>
      <rect x="47" y="47" width="37" height="37" rx="3" fill="white" fillOpacity="0.9"/>
    </svg>
  );
}
...
function VSCodeIcon() {
  return (
    <svg width="20" height="20" viewBox="0 0 100 100" fill="none">
      <path d="M74 6L38 45 17 28 6 34l19 16L6 66l11 6 21-17 36 39 17-8V14L74 6z" fill="#007ACC"/>
      <path d="M74 6L38 45l-21-17-11 6 19 16-19 16 11 6 21-17 36 39 17-8V14L74 6zm0 15l-1 58-29-29 30-29z" fill="white" fillOpacity="0.3"/>
    </svg>
  );
}
